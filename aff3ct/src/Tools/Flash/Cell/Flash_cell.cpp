...
namespace aff3ct
{
	namespace tools
	{
		Flash_cell ::Flash_cell(const unsigned cell_type) : n_threshold_indexes(cell_type - 1),
															n_levels(cell_type)
		{
		...
		void Flash_cell ::init_gray(const unsigned cell_type)
		{
			unsigned gray_i;
		...
		unsigned Flash_cell ::get_level_index(const unsigned symbol)
		{
			return level_index_map[symbol];
		}

		unsigned Flash_cell ::get_symbol(const unsigned level_index)
		{
			return symbol_map[level_index];
		}

		std::vector<unsigned> Flash_cell ::get_threshold_indexes(const unsigned page_type)
		{
			return threshold_indexes[page_type];
		}

		unsigned Flash_cell ::get_n_pages() const
		{
			return n_pages;
		}

		unsigned Flash_cell ::get_n_levels() const
		{
			return level_index_map.size();
		}

		unsigned Flash_cell ::get_n_threshold_indexes() const
		{
