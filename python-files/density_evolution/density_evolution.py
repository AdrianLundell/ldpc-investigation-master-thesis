...
def symmetric_density_evolution(cdf, f_grid, g_grid, rho_coeffs, lambda_coeffs, n_iter=50, tol=1e-3, plot=False, prnt=False):
    ...
    error = np.inf
    while True:
        x1 = de_utils.gamma(pl, f_grid, g_grid)
        x2 = de_utils.rho(x1, rho_coeffs)
        x3 = de_utils.gamma_inv(x2, f_grid, g_grid)
        x4 = de_utils.lambd(x3, lambda_coeffs)
        pl = de_utils.conv(x4, p0)
...
if __name__ == "__main__":
    t0 = time.time()
    n_grid = 1024
    rho_coeffs = np.array([0, 1.1203e-04, 0.0023, 0.0056, 0.0092, 0.0126, 0.0153, 0.0172, 0.0185,
                          0.0193, 0.0203, 0.0220, 0.0235, 0.0253, 0.0267, 0.0251, 0.0240, 0.0195, 6.4078e-04, 0.7129])
    lambda_coeffs = np.array([0, 0.9464, 0.0325, 0.0033, 0.0022, 0.0018, 0.0015, 0.0013, 0.0012, 0.0011, 0.0010,
                             9.8768e-04, 9.4196e-04, 9.0299e-04, 8.6939e-04, 8.4013e-04, 8.1443e-04, 7.9168e-04, 7.7141e-04, 7.5322e-04])
    ...
    def eval(x):
        f_grid, g_grid, pdf = de_utils.init_pdf(x, n_grid)
